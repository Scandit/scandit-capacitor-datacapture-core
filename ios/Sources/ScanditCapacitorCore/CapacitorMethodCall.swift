...
public class CapacitorMethodCall: FrameworksMethodCall {
    ...
    public var method: String {
        pluginCall.options["methodName"] as? String ?? "unknown"
    }

    public func argument<T>(key: String) -> T? {
        guard let value = pluginCall.options[key] else {
            return nil
        }
    ...
    public func hasArgument(key: String) -> Bool {
        pluginCall.options.keys.contains(key)
    }

    public func arguments() -> [String: Any] {
        var result: [String: Any] = [:]
        for (key, value) in pluginCall.options {
