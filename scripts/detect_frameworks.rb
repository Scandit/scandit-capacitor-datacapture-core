def podspec_path_for_pod(pod_name, current_path)
    ...
    if File.exist?(File.join(archive_dir, 'frameworks'))
        return File.join(archive_dir, 'frameworks', 'shared', 'ios', pod_name)
    end

    archive_temp_dir = File.expand_path(File.join('..', '..', '..', '..'))
    if File.exist?(File.join(archive_temp_dir, 'frameworks'))
        File.join(archive_temp_dir, 'frameworks', 'shared', 'ios', pod_name)
    else
